  ...
  const filteredUsers = useMemo(() => {
    const list = users.filter((u) => {
      ...
      if (acquisitionFilter !== "all") {
        const medium = u.acquisition?.medium || "none";
        const source = (u.acquisition?.source || "").toLowerCase();
        const isPaid = medium === "cpc" || medium === "paid_social" || source.includes("ads");

        if (acquisitionFilter === "paid" && !isPaid) return false;
        if (acquisitionFilter === "organic" && isPaid) return false;
        if (acquisitionFilter === "admin" && source !== "admin") return false;
      }
      ...
      if (searchQuery.trim()) {
        ...
        const matchSource = (u.acquisition?.source || "").toLowerCase().includes(q);
        const matchCampaign = (u.acquisition?.campaign || "").toLowerCase().includes(q);
        return matchName || matchEmail || matchUsername || matchUid || matchSource || matchCampaign;
      }
      return true;
    });
      ...
      <div className="rounded-[32px] bg-[#14161d] border border-white/10 p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <div className="relative sm:col-span-2 lg:col-span-2">
            ...
            <input
              type="text"
              placeholder="Buscar por nome, email, username, UID ou UTM..."
              value={searchQuery}
          ...
          <div>
            ...
            >
              ...
              <option value="paid">Tráfego Pago (Anúncios/CPC)</option>
              <option value="organic">Orgânico / Direto</option>
              <option value="admin">Criado por Admin</option>
            </select>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-white/5">
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((u) => {
                  ...
                  const isPaid = u.acquisition?.medium === "cpc" || u.acquisition?.medium === "paid_social" || (u.acquisition?.source || "").toLowerCase().includes("ads");
                  const isAdminCreated = u.acquisition?.source === "admin";
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5">
                        {isPaid ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-300 text-[10px] font-semibold border border-amber-500/25 capitalize">
                            <span className="w-1.5 h-1.5 rounded-full bg-amber-400"></span>
                            <span>{u.acquisition?.source || "Anúncio"}</span>
                          </span>
                        ) : isAdminCreated ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-500/15 text-purple-300 text-[10px] font-semibold border border-purple-500/25">
                            <span className="w-1.5 h-1.5 rounded-full bg-purple-400"></span>
                            <span>Admin</span>
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/5 text-gray-300 text-[10px] font-medium border border-white/10 capitalize">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
                            <span>{u.acquisition?.source || "Orgânico"}</span>
                          </span>
                        )}
                      </td>

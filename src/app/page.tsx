...
export default function HomePage() {
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      {/* ==========================================
          BARRA FIXA DE FILTROS POR PLATAFORMA & ROLETA
      ========================================== */}
      <div className="sticky top-16 z-20 py-2.5 px-3 sm:px-4 rounded-2xl bg-[#0f1218]/95 border border-[#242a36] backdrop-blur-xl shadow-xl flex items-center justify-between gap-3 overflow-x-auto scrollbar-none">
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 font-mono text-xs">
          <span className="text-[11px] text-neutral-400 uppercase tracking-wider flex items-center gap-1.5 font-bold mr-1">
          ...
          {[
            ...
            <Link
              key={p.label}
              href={p.href}
              className="px-2.5 sm:px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-neutral-300 hover:text-white border border-white/5 hover:border-white/20 transition-all font-medium whitespace-nowrap active:scale-95"
            >
              {p.label}
            </Link>
          ))}
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 text-xs px-3.5 py-1.5 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 hover:border-amber-400 font-mono font-bold transition-all shadow-sm active:scale-95 cursor-pointer shrink-0"
          title="Roleta Gamer: Sortear jogo aleatório"
        >

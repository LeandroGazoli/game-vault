...
interface GNewsCardProps {
  item: GNewsArticle;
  onImport: (item: GNewsArticle, rewriteWithAI: boolean) => void;
  isRewriting?: boolean;
}
...
export default function GNewsCard({
  ...
  return (
    <div className="p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-violet-500/30 transition-all space-y-3 group">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex items-center justify-between text-[10px] text-zinc-400 gap-2 flex-wrap">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="px-2 py-0.5 rounded-full bg-violet-500/10 text-violet-400 border border-violet-500/20 font-bold">
                {item.source?.name || "GNews"}
              </span>
              <span className="px-2 py-0.5 rounded-full bg-white/5 text-zinc-300 border border-white/10 uppercase text-[9px] font-mono">
                {item.lang}
              </span>
            </div>
          ...
          <h4 className="text-xs sm:text-sm font-bold text-white group-hover:text-violet-300 transition-colors line-clamp-2">
            {item.title}
          </h4>
      ...
      <div className="pt-2 flex items-center justify-between border-t border-white/5 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title="Importa o texto original para você editar manualmente"
          >
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-violet-500 hover:bg-violet-400 text-white text-xs font-black transition-all flex items-center gap-1.5 shadow-md hover:scale-105 active:scale-95 disabled:opacity-50 cursor-pointer"
